...
public class Date {
   ...
   public boolean dayOk(int day, String month) {
      ...
      int[] VALID_DAYS = new int[] { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
      int index = getMonthIndex(month);
      return day <= VALID_DAYS[index] ? true : false;
   }
   ...
   public int getMonthIndex(String monthStr) {         
      ...
      for (int i = 0; i < VALID_MONTHS.length; i++){
         if (VALID_MONTHS[i].equals(monthStr)) {
            return i;
         }
      }
      return 0;
